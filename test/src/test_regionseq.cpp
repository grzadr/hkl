...
using General::gen_framed;
using General::passed_str;
using General::Stats;

using HKL::FASTAReader;

General::Stats EvalRegionSeq::eval_regionseq(bool verbose) {
  Stats stats;

  cout << General::gen_framed("Evaluating RegionSeq") << "\n\n";

  stats(check_basic(verbose));
  stats(check_get_seq(verbose));
  stats(check_read_fasta(verbose));
